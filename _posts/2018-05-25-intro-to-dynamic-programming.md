...
- the intial state is dp[s.length][p.length] and it is true, empty string matches empty pattern.
...
3. dp[0][0] is 1. It's weired I know, but when you think about it, dp[x][x] should never be zero.
...
Divide and Conquer also works by dividing the problem into subproblems, but don't confuse it with DP. DP acutually emphasize on using memoization to solve overlapping subproblems efficiently. Remember:
